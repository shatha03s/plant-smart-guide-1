...
client = OpenAI(api_key=OPENAI_API_KEY) if (OPENAI_API_KEY and ENABLE_OPENAI) else None
...
def import_plants_json():
    ...
    for item in data:
        ...
        search_text = build_search_text(name_ar, name_en, scientific_name, category, watering, sunlight, soil, description, problems, care_tips, aliases_text)
        rows.append((name_ar, name_en, scientific_name, category, watering, sunlight, soil, description, problems, care_tips, aliases_text, search_text))
    conn = db()
    cur = conn.cursor()
    cur.executemany("""
    INSERT INTO plants
    (name_ar, name_en, scientific_name, category, watering, sunlight, soil, description, problems, care_tips, aliases, search_text)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, rows)
    conn.commit()
    conn.close()
...
def tokenize(text: str) -> List[str]:
    return [t for t in normalize_arabic(text).replace("،", " ").replace(",", " ").split() if len(t) > 1]
...
def local_rag_search(query: str, limit: int = 7) -> Tuple[List[sqlite3.Row], int]:
    ...
    for p in rows:
        ...
        text = safe(p, "search_text") or build_search_text(*[safe(p, k) for k in p.keys()])
        score = 0
        if q_norm and q_norm in name_ar: score += 120
        if q_norm and q_norm in aliases: score += 115
        if q_norm and q_norm in name_en: score += 100
        if q_norm and q_norm in scientific: score += 95
        if q_norm and q_norm in text: score += 55
        for tok in q_tokens:
            if tok in name_ar or tok in aliases: score += 25
            elif tok in text: score += 8
        ratio = max(
...
العناية: {safe(p,'care_tips')}""" for p in context[:5]
    ])
    prompt = f"""
...
"""
    try:
        res = client.chat.completions.create(
            model=OPENAI_MODEL,
            messages=[
                {"role": "system", "content": "أنت مساعد مختص بالنباتات يجيب بالعربية بأسلوب واضح ورسمي."},
...
def openai_vision_answer(path: str, message: str) -> str:
    ...
    try:
        ...
        res = client.chat.completions.create(
            model=OPENAI_MODEL,
            messages=[
                {"role": "system", "content": "أنت خبير نباتات. حلل الصورة بالعربية باختصار وبنقاط عملية."},
                {"role": "user", "content": [
                    {"type": "text", "text": f"حلل صورة النبات. اذكر النبات المحتمل، حالته، المشكلة الظاهرة، ونصائح العناية. سؤال المستخدم: {message or 'لا يوجد سؤال'}"},
...
def library():
    ...
    categories = cur.execute("SELECT DISTINCT category FROM plants WHERE category IS NOT NULL AND category != '' ORDER BY category").fetchall()
    total = cur.execute("SELECT COUNT(*) FROM plants").fetchone()[0]
    conn.close()
    return render_template("library.html", plants=plants, categories=categories, selected_category=category, q=q, total=total)
...
def save_conversation(user_message, bot_reply, source):
    try:
        conn = db()
        conn.execute("INSERT INTO conversations (user_message, bot_reply, source) VALUES (?, ?, ?)", (user_message, bot_reply, source))
        conn.commit()
        conn.close()
    except Exception as e:
...
def analyze_image():
    try:
        ...
        try:
            conn = db()
            conn.execute("INSERT INTO image_analyses (filename, user_message, bot_reply) VALUES (?, ?, ?)", (filename, message, reply))
            conn.commit()
            conn.close()
        except Exception as e:
